...
class RestaurantControllerTest {
	...
	void create() throws Exception {
		given(restaurantService.addRestaurant(null)).will(invocation -> {
			Restaurant restaurant = invocation.getArgument(0);
			return Restaurant.builder().id(1004L).name(restaurant.getName()).address(restaurant.getAddress()).build();
		});

		mvc.perform(post("/restaurant").contentType(MediaType.APPLICATION_JSON)
				.content("\"name\":\"Joonsung\",\"address\":\"AnYang\"")).andExpect(status().isCreated())
				.andDo(print());
	}

	public void update() throws Exception {

		mvc.perform(patch("/restaurant/1004").contentType(MediaType.APPLICATION_JSON)
				.content("{\"name\":\"Joonsung Bar\",\"address\":\"SooWon\"}")).andExpect(status().isOk());
		verify(restaurantService.updateRestaurant(1004L, "Joonsung Bar", "SooWon"));
	}
